...
// step != 0 is necessary
    ...
    case MULTIPLICATIVE:
        ...
        case QUADRATIC_REHASH:
            return (goldIndx + STEP*probe + probeStepSz*probe*probe)%usedSize;
        case DOUBLE_HASHING:
...
void search()
{
    ...
    while (fgets(strg, 90, keys) != NULL)
    {
        el= strlen(strg)-1;
        strg[el] = '\0';
        dataP = (char *) malloc(strlen(strg)*sizeof(char)+1);
        assert(dataP != NULL);
        strcpy(dataP, strg);
        reTry = 0;
...
int main(void)
{
    ...
    int el, reTry, rehash=0, filled=0,num;
    initialise(TBLSZ);
